...
## doll.
##
##   Godot --headless --path . -s res://tools/validate_magnet_train.gd
...
func _finish() -> void:
	if _failures.is_empty():
		print("PASS magnet train: the errand's doors, the Copycat before her sprite, the Fan Club doll and both sealed station platforms verified.")
		quit(0)
		return
	for failure: String in _failures:
		printerr(failure)
	printerr("FAIL magnet train: %d problems." % _failures.size())
	quit(1)

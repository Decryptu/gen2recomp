...
func run(r: RefCounted) -> void:
	...
	if data == null:
		_r.fail("Gold cache is unavailable. Import roms/gold.gbc first.")
		return
	var route: Gen2WorldMap = data.world_map(MAP_GROUP, MAP_NUMBER)
	if route == null:
		_r.fail("Route 30 map 26/1 is missing from the Gold cache.")
		return
...
func _verify_imported_record(data: GameData, event: Dictionary, trainer: Dictionary) -> void:
	...
	_r.check(int(event.get("sight_range", -1)) == 4, "Joey's sight range is not four.")
	_r.check(
...
func _verify_runtime_flow(data: GameData, trainer_record: Dictionary) -> void:
	...
	_r.check(joey.facing == Gen2WorldSprite.FACING_LEFT, "Joey does not face left.")

	var movement: Dictionary = world.move_result(Vector2i.LEFT)
	_r.check(bool(movement.get("ok", false)), "The source approach setup cell cannot move left.")
	...
	_r.check(approach.get("direction", Vector2i.ZERO) == Vector2i.LEFT, "Approach direction is wrong.")
	...
	_r.check(plan.get("path", []) == [Vector2i.LEFT], "Joey does not stop one cell away.")
	_r.check(
	...
	_r.check(joey.facing == Gen2WorldSprite.FACING_LEFT, "Joey's final facing is wrong.")
	_r.check(world.player_facing == Gen2WorldSprite.FACING_RIGHT, "Player final facing is wrong.")
	...
	_r.check(joey.cell == Vector2i(5, 29), "Joey's written approach cell did not survive reload.")

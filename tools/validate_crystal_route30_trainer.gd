...
func _initialize() -> void:
	...
	if data == null:
		_fail("Crystal cache is unavailable. Import roms/crystal.gbc first.")
		_finish()
		return
	var route: Gen2WorldMap = data.world_map(MAP_GROUP, MAP_NUMBER)
	if route == null:
		_fail("Route 30 map 26/1 is missing from the Crystal cache.")
		_finish()
		return
...
func _verify_imported_record(data: GameData, event: Dictionary, trainer: Dictionary) -> void:
	...
	_check(int(event.get("sight_range", -1)) == 3, "Joey's sight range is not three.")
	_check(
...
func _verify_runtime_flow(data: GameData, trainer_record: Dictionary) -> void:
	...
	_check(joey.facing == Gen2WorldSprite.FACING_RIGHT, "Joey does not face right.")

	var movement: Dictionary = world.move_result(Vector2i.RIGHT)
	_check(bool(movement.get("ok", false)), "The source approach setup cell cannot move right.")
	...
	_check(sight_source.get("direction", Vector2i.ZERO) == Vector2i.RIGHT, "Joey's sight direction is wrong.")
	...
	_check(approach.get("direction", Vector2i.ZERO) == Vector2i.RIGHT, "Approach direction is wrong.")
	...
	_check(plan.get("path", []) == [Vector2i.RIGHT], "Joey does not stop one cell away.")
	_check(
	...
	_check(joey.cell == Vector2i(3, 28), "Joey did not stop before the player.")
	...
	_check(joey.facing == Gen2WorldSprite.FACING_RIGHT, "Joey's final facing is wrong.")
	_check(world.player_facing == Gen2WorldSprite.FACING_LEFT, "Player final facing is wrong.")
	...
	_check(joey.cell == Vector2i(3, 28), "Joey's written approach cell did not survive reload.")
...
func _finish() -> void:
	if _failures.is_empty():
		print("PASS crystal Route 30 trainer: imported record and initial encounter verified.")
		quit(0)
		return

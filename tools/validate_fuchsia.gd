...
func _finish() -> void:
	if _failures.is_empty():
		print("PASS fuchsia: the four routes, the trainers each walk owes, the gate, the city and Janine's maze verified.")
		quit(0)
		return
	for failure: String in _failures:
		printerr(failure)
	printerr("FAIL fuchsia: %d problems." % _failures.size())
	quit(1)

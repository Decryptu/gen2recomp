...
point is the centre of its 16x16 icon. `Pokedex_GetArea` is the third frame over
the same two pieces: it holds its region in the cursor byte the way the source
reuses `wTownMapCursorLandmark`, and the landmarks it marks are
`Gen2WorldEncounter.nests`, which is `FindNest` over one region's own wild
tables (`engine/overworld/wildmons.asm`).
...
| `render/town_map_page.gd` | `_TownMap`'s, the Pokegear MAP card's and the dex AREA screen's region map |
...
| Region map | The two region tilemaps pin the graphic they are drawn out of: every one of their 720 cells names a tile inside `TownMapGFX`'s 48, and each ends on its own `-1`. `PokedexNestIconGFX` is the tile behind the Kanto map's terminator and is identified by content, every row inked and every plane byte its own bit reversal. The palette map is checked against the six palettes it selects between, and the palette run by the off-white all six open on. The landmark table checks both ends, that only `LANDMARK_SPECIAL` sits at (0,0), and that every name pointer stays in the table's own bank |
